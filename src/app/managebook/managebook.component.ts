...
export class ManagebookComponent {
  ...
  bookUpdation(){
    this.service.bookDetails().subscribe((data)=>{
        ...
        if(find){
          ...
          this.service.updateBookDetails("http://localhost:3000/bookDetails"+"/"+this.storeID,formvalue);
  ...
  deleteBook(id:any){
    this.http.get<any>("http://localhost:3000/bookDetails").subscribe((data)=>{
      const find=data.find((value:any)=>{
          this.storeData=value;
          return value.id==id;
      });
      if(find){
        this.http.delete<any>("http://localhost:3000/bookDetails"+"/"+id).subscribe(()=>{
          alert("Book Deleted");
